package com.ogoons.hagotimelineview
 ...
 */
class HagoTimelineView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private fun init(attrs: AttributeSet) {
        ...
        markerMargin = typedArray.getDimensionPixelSize(R.styleable.HagoTimelineView_markerMargin, Util.dpToPx(context, DEF_MARKER_MARGIN_DP))
        lineType = when (typedArray.getInt(R.styleable.HagoTimelineView_lineType, DEF_LINE_TYPE)) {
            0 -> LineType.BEGIN
            1 -> LineType.MIDDLE
            2 -> LineType.END
            3 -> LineType.ONLY
        ...
        lineOrientation = if (typedArray.getInt(R.styleable.HagoTimelineView_lineOrientation, DEF_LINE_ORIENTATION) == 0) LineOrientation.HORIZONTAL else LineOrientation.VERTICAL
        lineStyle = if (typedArray.getInt(R.styleable.HagoTimelineView_lineStyle, DEF_LINE_STYLE) == 0) LineStyle.SOLID else LineStyle.DASHED
        lineWidth = typedArray.getDimensionPixelSize(R.styleable.HagoTimelineView_lineWidth, Util.dpToPx(context, DEF_LINE_HEIGHT_DP))
        lineColor = typedArray.getColor(R.styleable.HagoTimelineView_lineColor, ContextCompat.getColor(context, android.R.color.darker_gray))
        dashWidth = typedArray.getDimensionPixelSize(R.styleable.HagoTimelineView_dashWidth, Util.dpToPx(context, DEF_DASH_WIDTH_DP))
        dashGap = typedArray.getDimensionPixelSize(R.styleable.HagoTimelineView_dashGap, Util.dpToPx(context, DEF_DASH_GAP_DP))
        markerInCenter = typedArray.getBoolean(R.styleable.HagoTimelineView_markerInCenter, true)
        typedArray.recycle()
